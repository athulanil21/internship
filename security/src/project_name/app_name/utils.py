...
def ping(ping_host):
    print(f"\n🛰️ Pinging {ping_host}")
    try:
        output = subprocess.check_output(["ping", ping_host], stderr=subprocess.STDOUT)
        print(output.decode())
    except subprocess.CalledProcessError as e:
        print(f"Ping failed:\n{e.output.decode()}")


def ssl_info(ssl_domain):
    print(f"\n🔐 SSL Info for {ssl_domain}")
    ctx = ssl.create_default_context()
    try:
        with ctx.wrap_socket(socket.socket(), server_hostname=ssl_domain) as s:
            s.settimeout(5)
            s.connect((ssl_domain, 443))
            cert = s.getpeercert()
...
def check_http_security_headers(securityheaders_domain):
    print(f"\n🛡️ HTTP Security Headers for {securityheaders_domain}")
    try:
        response = requests.get(f"https://{securityheaders_domain}", timeout=5)
...
def check_ssllabs(ssllabs_domain):
    print(f"\n📊 SSL Labs Grade for {ssllabs_domain}")
    api_url = "https://api.ssllabs.com/api/v3/analyze"

    try:
        params = {"host": ssllabs_domain, "publish": "off", "startNew": "on", "all": "done"}
        ...
        while analysis.get("status") in ["DNS", "IN_PROGRESS", "INITIALIZING"]:
            ...
            time.sleep(10)
            response = requests.get(api_url, params={"host": ssllabs_domain})
            analysis = response.json()
...
def detect_waf_and_cloudflare(cloudflare_domain, dns=None):
    print(f"\n🛡️ WAF / CDN / Cloudflare Detection for {cloudflare_domain}")
    try:
        response = requests.get(f"https://{cloudflare_domain}", timeout=5)
        headers = response.headers
        ...
        try:
            cname = dns.resolver.resolve(cloudflare_domain, 'CNAME')
            for r in cname:
...
def ns_lookup(ns_lookup_domain):
    print(f"\n🔍 NSLookup for {ns_lookup_domain}")
    try:
        result = dns.resolver.resolve(ns_lookup_domain, 'A')
        for ip in result:
            print(f"A Record: {ip}")
    except Exception as e:
